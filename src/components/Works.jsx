...
const StyledImages=(props)=>{
  return (
    <div className='bg-[#A1128D] relative h-[600px]'>
      ...
      <div className='bg-no-repeat w-64  sm:w-96  rounded-lg md:hover:w-[450px] hover:z-20 hover:cursor-pointer border-2 border-black rotate-12 absolute top-32 right-0 md:left-64  hover:rotate-0'>
        <video autoPlay muted loop id="myVideo">
            <source src="./Projects/projectOneVid2.mp4" type="video/mp4"/>
        </video>
      </div>
      <a href={props.link}>
        <div className='bg-[url("./Projects/projectLogo.png")] bg-center bg-cover bg-no-repeat w-32 h-32 rounded-lg  border-4 border-black absolute top-32 right-1/3 md:left-64 '></div>
      </a>
    </div>
  )
}
